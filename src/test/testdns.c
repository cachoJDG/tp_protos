...
// /* Saca un trabajo de la cola; devuelve 0 si extrajo uno, o -1 si hay que terminar */
...
// /* Inserta un trabajo en la cola; asume espacio suficiente */
...
//     /* 2) Encolo los 2 trabajos */
//     push_job(argv[1], argv[2]);
//     push_job(argv[3], argv[4]);

//     /* 3) Espero a que se procesen todos los trabajos */
//     pthread_mutex_lock(&queue_mtx);
//     while (jobs_pending > 0) {
//         pthread_cond_wait(&done_cnd, &queue_mtx);
//     }
//     /* 4) Señalizo a los hilos que no habrá más trabajos */
//     stop_pool = 1;
//     pthread_cond_broadcast(&queue_cnd);
//     pthread_mutex_unlock(&queue_mtx);

//     /* 5) Espero a que todos los hilos terminen */
//     for (int i = 0; i < POOL_SIZE; i++) {
//         pthread_join(pool[i], NULL);
//     }

//     /* 6) Cierro todos los sockets recopilados */
